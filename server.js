...
// global weatherData and forecastData
let weatherData = {};
let forecastData = {};
...
app.post("/weather", async (req, res) => {
  ...
  forecastData = await openForecastReq(
...
app.get("/weather/show", (req, res) => {
  res.render("weather/show", { weatherData, forecastData });
});

const openWeatherReq = async (zip, code) => {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?zip=${zip},${code}&units=imperial&appid=${process.env.API_KEY}`
  );
...
const convertToMetric = () => {
  const tempC = parseFloat((((weatherData.main.temp - 32) * 5) / 9).toFixed(2));
  weatherData.main.celsius = tempC;
  const windKph = parseFloat((weatherData.wind.speed * 1.60934).toFixed(2));
  weatherData.wind.kph = windKph;
...
const convertForecastDates = () => {
  ...
  forecastData.list.forEach((unixDt) => {
    unixDt.dt = unixToDate(unixDt.dt);
  });
};

const getCountryCode = async (country) => {
  const foundCountry = await countriesObject.find(
    (c) => c.countryNameEn === country
  );
